...
{
	vec3 s = pow(getTexel(vec2(tc.x, 1.0 - tc.y)).xyz, vec3(2.2));
	return s;
}
...
vec4 Process(vec4 color)
{
	...
	vec3 oricol = getTexel(vec2(q.x,q.y)).xyz;
	vec3 oricol_inv = getTexel(vec2(nq.x,nq.y)).xyz;
	...
	x+=o*0.25;
    col.r = blur(vec2(x+uv.x+0.0009,uv.y+0.0009),iResolution.y/2000.0).x+0.02;
    col.g = blur(vec2(x+uv.x+0.0000,uv.y-0.0011),iResolution.y/2000.0).y+0.02;
    col.b = blur(vec2(x+uv.x-0.0015,uv.y+0.0000),iResolution.y/2000.0).z+0.02;
	float i = clamp(col.r*0.299 + col.g*0.587 + col.b*0.114, 0.0, 1.0 );
	
	// Glow
	vec3 glow = (12.5*i*i)*pow(clamp(blur(vec2(x+uv.x+0.2*sin(uv.x + 10.0*Timer)*0.012,uv.y + 0.2*sin( uv.y + 7.3*Timer)*0.012),4.0)-0.3,0.0,1.0),vec3(5.0));
	...
	// Ghosting
    float ghs = 0.6;
	vec3 r = blur(vec2(x-0.014*1.0, -0.027)+0.003*vec2( 0.35*sin(1.0/7.0 + 35.0*uv.y + 0.9*Timer), 0.55*sin( 2.0/7.0 + 10.0*uv.y + 2.37*Timer) )+vec2(uv.x+0.001,uv.y+0.001),5.5+1.3*sin( 3.0/9.0 + 31.0*uv.y + 1.70*Timer)).xyz*vec3(0.5,0.25,0.25);
	vec3 g = blur(vec2(x-0.019*1.0, -0.020)+0.003*vec2( 0.35*sin(1.0/9.0 + 35.0*uv.y + 0.5*Timer), 0.55*sin( 2.0/9.0 + 10.0*uv.y + 2.50*Timer) )+vec2(uv.x+0.000,uv.y-0.002),5.4+1.3*sin( 3.0/3.0 + 71.0*uv.y + 1.90*Timer)).xyz*vec3(0.25,0.5,0.25);
	vec3 b = blur(vec2(x-0.017*1.0, -0.003)+0.003*vec2( 0.35*sin(2.0/3.0 + 35.0*uv.y + 0.7*Timer), 0.55*sin( 2.0/3.0 + 10.0*uv.y + 2.63*Timer) )+vec2(uv.x-0.002,uv.y+0.000),5.3+1.3*sin( 3.0/7.0 + 91.0*uv.y + 1.65*Timer)).xyz*vec3(0.25,0.25,0.5);
